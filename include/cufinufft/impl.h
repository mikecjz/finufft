...
int cufinufft_deconvolve_impl(cuda_complex<T> *d_fk, cuda_complex<T> *d_fw, cufinufft_plan_t<T> *d_plan) {
  ...
  case 2: {
    ...
    if (type == 2) ier = cufinufft2d2_deconvolve<T>(d_fk, d_fw, d_plan);
    if (type == 3) ier = FINUFFT_ERR_METHOD_NOTVALID;
  } break;
  case 3: {
    if (type == 1) ier = cufinufft3d1_deconvolve<T>(d_fk, d_fw, d_plan);
    if (type == 2) ier = cufinufft3d2_deconvolve<T>(d_fk, d_fw, d_plan);
    if (type == 3) ier = FINUFFT_ERR_METHOD_NOTVALID;
  } break;
  }
  return ier;
